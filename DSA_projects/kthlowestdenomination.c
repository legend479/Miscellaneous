...
int InHeap(HeapSentinel* H, HeapElementType ele)
{
    for(int i=0; i<H->NumOfEle; ++i)
    {
        if(H->HeapArray[i]==ele)return 1;
    }
    return 0;
}
//public helper
...
int main()
{
    ...
    for(int i=0; i<K-1; ++i)
    {
        for(int i=0; i<N; ++i)
        {
            if(!InHeap(H,A[i]+FindRoot(H)))Insert(H,A[i]+FindRoot(H));
        }
        ExtractRoot(H);
    }
    printf("%ld\n",FindRoot(H));
