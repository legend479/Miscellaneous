...
void siftdown(HeapSentinel* H, int pos)//Heap dependent
{
    ...
    else if(RightChild(pos)>=H->NumOfEle)MaxChild = LeftChild(pos);
    else MaxChild = H->HeapArray[LeftChild(pos)]>H->HeapArray[RightChild(pos)]?LeftChild(pos):RightChild(pos);
    
    if(H->HeapArray[pos] < H->HeapArray[MaxChild])
    {
        //swap
        H->HeapArray[pos] = H->HeapArray[pos] ^ H->HeapArray[MaxChild];
        H->HeapArray[MaxChild] = H->HeapArray[pos] ^ H->HeapArray[MaxChild];
        H->HeapArray[pos] = H->HeapArray[pos] ^ H->HeapArray[MaxChild];
